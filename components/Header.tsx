...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-gradient-to-r from-gray-900 via-blue-900 to-gray-900 text-white shadow-2xl">
      {/* Верхняя полоска */}
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 py-1 px-4 text-center text-xl">
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-20 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Кнопка поиска */}
            <button className="p-2.5 rounded-full bg-gray-800/50 hover:bg-gray-800 text-gray-300 hover:text-white transition-colors">
            ...
            {/* Мобильное меню */}
            <button className="md:hidden p-2.5 rounded-lg bg-gray-800 text-gray-400 hover:text-white">
      ...
      {/* Мобильная навигация */}
      <div className="md:hidden bg-gradient-to-r from-gray-800 to-gray-900 border-t border-gray-700">
          ...
          <div className="flex justify-around">
            {categories.map((category) => (
              <Link
                key={category.id}
                href={category.path}
                className={`px-4 py-2 rounded-lg text-sm font-medium bg-gradient-to-r ${category.color} text-white`}
              >
                {category.name}
              </Link>
            ))}
