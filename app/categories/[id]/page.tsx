...
import { ArrowLeft, Users, Clock, BookOpen, Code2 } from 'lucide-react';

interface CategoryPageProps {
  params: Promise<{ id: string }>; // Добавляем Promise
}
...
export async function generateStaticParams() {
  console.log('Генерируем статические пути для категорий:', CATEGORIES.map(c => c.id));
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  // Извлекаем params с await
  const { id } = await params;
  const categoryId = parseInt(id);
  
  console.log(`Загружаем страницу категории ID: ${categoryId}`);
  
  // Проверяем, что categoryId валидный (1, 2 или 3)
  const category = CATEGORIES.find(cat => cat.id === categoryId);
  if (!category) {
    console.log(`Категория ${categoryId} не найдена, показываем 404`);
    notFound();
  }
  ...
  try {
    ...
    console.log(`Найдено курсов для категории ${categoryId}: ${courses.length}`);
    ...
    console.error(`Ошибка при загрузке курсов для категории ${categoryId}:`, error);
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto max-w-6xl px-4 py-16">
          <div className="text-center">
            ...
            <p className="text-gray-500 mb-6">Не удалось загрузить курсы для этой категории</p>
            <Link 
              href="/courses" 
